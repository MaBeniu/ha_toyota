...
def _vehicle_has_climate_capability(vehicle: Vehicle) -> bool:
    ...
    try:
        ...
        caps = getattr(info, "extended_capabilities", None)

        return bool(
            getattr(features, "climate_start_engine", False)
            or getattr(caps, "climate_capable", False)
            or getattr(caps, "econnect_climate_capable", False)
            or getattr(caps, "remote_engine_start_stop", False)
        )
